...
export async function onCompleteActivity(address: string) {
  try {
    ...
    const options = {
      ...
      body: `{"walletId":"24466965-893d-43bf-9cc4-e3ad29bddb61","contractAddress":"0x861ACCC81DE9A158c5a0660b344870E8D4a3A11A","operations":[{"argumentsValues":["${address}"],"messageValue":0,"functionSignature":"completeActivity(address)"}]}`,
    };
...
export async function onSetEligibility(address: string) {
  try {
    console.log(address);
    const body = `{"operations":[{"argumentsValues":["${address}",true],"functionSignature":"setEligibility(address, bool)","messageValue":0}],"walletId":"24466965-893d-43bf-9cc4-e3ad29bddb61","contractAddress":"0x861ACCC81DE9A158c5a0660b344870E8D4a3A11A"}`;
    console.log(body);
    const options = {
      method: "POST",
      headers: {
    ...
    const res = await response.json();
    console.log("onSetEligibility", res);
    return res;
  } catch (error) {
    console.log(error);
    throw error;
